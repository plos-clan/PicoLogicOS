...
static void say_hello() {
  ...
  if (id_pos && version_pos) {
    ...
    *end = '\0';
    ...
    wws(STDERR, "! PicoLogicOS sends its greetings from your userspace!", 54);
    write(STDERR, "\033[m\n\033[32m", 9);
    usleep(250000);
...
void _start() {
  ...
  struct utsname buf;
  if (uname(&buf) == 0) {
    wws(STDERR, "Oops! PicoLogicOS kernel got lost in ", 37);
    wws(STDERR, buf.sysname, strlen(buf.sysname));
    wws(STDERR, "-land!", 6);
  } else {
    wws(STDERR, "Oops! PicoLogicOS kernel is running in userspace!", 49);
  }
  write(STDERR, "\033[m\n\033[33m", 9);
  usleep(250000);
