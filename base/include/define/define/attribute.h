...
#define _rest __restrict //+ PLOS Defination
...
#define __attr_pure        __attr(pure) //+ PLOS Defination

//+ PLOS Defination
#ifdef __clang__
#  define __attr_access(x) // 是的我们 clang 没有 access 属性
...
#define __nnull(...) __attr(nonnull(__VA_ARGS__)) //+ PLOS Defination
...
#define DEPRECATED __attr(deprecated)
...
// 函数返回内存地址，调用者获得内存所有权
#define ownership_returns(type)       __attr(ownership_returns(type)) //+ PLOS Defination
// 函数获取内存所有权并释放内存
#define ownership_takes(type, nparam) __attr(ownership_takes(type, nparam)) //+ PLOS Defination
// 函数获取内存所有权
#define ownership_holds(type, nparam) __attr(ownership_holds(type, nparam)) //+ PLOS Defination

// 强制函数内所有调用内联
#define FLATTEN __attr(flatten) //+ PLOS Defination

// 所在的段
#define SECTION(name) __attr(section(name)) //+ PLOS Defination
