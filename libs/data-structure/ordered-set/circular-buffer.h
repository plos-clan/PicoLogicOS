...
typedef struct cirbuf {
  byte  *buf;
  size_t size; /**< 循环缓冲区的最大大小 */
  size_t head; /**< 指向下一个读取的位置 */
  size_t tail; /**< 指向下一个写入的位置 */
  size_t len;  /**< 当前的元素个数 */
} *cirbuf_t;

typedef struct cirbuf CircularBuffer, Cirbuf;
...
API bool cirbuf_put8(cirbuf_t cirbuf, u8 data);
API bool cirbuf_put16(cirbuf_t cirbuf, u16 data);
API bool cirbuf_put32(cirbuf_t cirbuf, u32 data);
API bool cirbuf_put64(cirbuf_t cirbuf, u64 data);
