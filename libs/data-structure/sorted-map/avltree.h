...
#if !defined(COMPILE_DATA_STRUCTURE_LIBRARY) && defined(AVL_IMPLEMENTATION)
#  define API static
#else
#  define API dlimport
#endif
 ...
 *\return 新的AVL树节点指针
 */
API avltree_t avltree_alloc(int32_t key, void *value) __THROW __attr_malloc;

/**
 *\brief 释放AVL树
 *\param[in] root 树的根节点
 */
API void avltree_free(avltree_t root) __THROW;
 ...
 */
API void avltree_free_with(avltree_t root, void (*callback)(void *)) __THROW;
 ...
 */
API void *avltree_get(avltree_t root, int32_t key) __THROW;
 ...
 */
API avltree_t avltree_get_node(avltree_t root, int32_t key) __THROW;
 ...
 */
API bool avltree_search(avltree_t root, void *value, int32_t *key) __THROW;
 ...
 */
API avltree_t avltree_search_node(avltree_t root, void *value) __THROW;
 ...
 */
API avltree_t avltree_min(avltree_t root) __THROW;
 ...
 */
API avltree_t avltree_max(avltree_t root) __THROW;
 ...
 *\return 更新后的树的根节点指针
 */
API avltree_t avltree_insert(avltree_t root, int32_t key, void *value) __THROW __wur;
 ...
 *\return 更新后的树的根节点指针
 */
API avltree_t avltree_delete(avltree_t root, int32_t key) __THROW __wur;
 ...
 *\return 更新后的树的根节点指针
 */
API avltree_t avltree_delete_with(avltree_t root, int32_t key, void (*callback)(void *)) __THROW
__wur;
 ...
 */
API void avltree_print_inorder(avltree_t root, int depth) __THROW;
 ...
 *\param[in] depth 递归深度（用于缩进打印）
 */
API void avltree_print_preorder(avltree_t root, int depth) __THROW;
 ...
 *\param[in] depth 递归深度（用于缩进打印）
 */
API void avltree_print_postorder(avltree_t root, int depth) __THROW;
...
API avltree_t avltree_alloc(int32_t key, void *value) noexcept {
...
API void avltree_free(avltree_t root) noexcept {
  if (root == null) return;
...
API void avltree_free_with(avltree_t root, void (*callback)(void *)) noexcept {
  if (root == null) return;
...
API void *avltree_get(avltree_t root, int32_t key) noexcept {
...
API avltree_t avltree_get_node(avltree_t root, int32_t key) noexcept {
  while (root != null && root->key != key) {
    if (key < root->key)
      root = root->left;
    else
...
API bool avltree_search(avltree_t root, void *value, int32_t *key) noexcept {
...
API avltree_t avltree_search_node(avltree_t root, void *value) noexcept {
...
API avltree_t avltree_min(avltree_t root) noexcept {
  while (root->left != null)
    root = root->left;
  return root;
}

API avltree_t avltree_max(avltree_t root) noexcept {
  while (root->right != null)
    root = root->right;
  return root;
}

API avltree_t avltree_insert(avltree_t root, int32_t key, void *value) noexcept {
...
API avltree_t avltree_delete(avltree_t root, int32_t key) noexcept {
  if (root == null) { return null; }
...
API avltree_t avltree_delete_with(avltree_t root, int32_t key, void (*callback)(void *)) noexcept {
  if (root == null) { return null; }
...
API void avltree_print_inorder(avltree_t root, int depth) noexcept {
...
API void avltree_print_preorder(avltree_t root, int depth) noexcept {
...
API void avltree_print_postorder(avltree_t root, int depth) noexcept {
